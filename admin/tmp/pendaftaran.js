...
$(document).ready(function(){
    ...
    function load_data(keyword) {
      $.ajax({
        type: 'GET',
        url: 'http://localhost/lite3/admin/pendaftaran/ajax?keyword='+keyword+'&t=1bdf2736be5c',
...
$(document).ready(function () {
  ...
  $('.poliklinik').selectator({
    ...
    load: function (search, callback) {
      ...
      $.ajax({
        url: 'http://localhost/lite3/admin/pendaftaran/ajax?show=poliklinik&s=' + encodeURIComponent(search) + '&t=1bdf2736be5c',
        type: 'GET',
        dataType: 'json',
  ...
  $('.dokter').selectator({
    ...
    load: function (search, callback) {
      if (search.length < this.minSearchLength) return callback();
      $.ajax({
        url: 'http://localhost/lite3/admin/pendaftaran/ajax?show=dokter&s=' + encodeURIComponent(search) + '&t=1bdf2736be5c',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
  ...
  $('.icd10').selectator({
    ...
    load: function (search, callback) {
      if (search.length < this.minSearchLength) return callback();
      $.ajax({
        url: 'http://localhost/lite3/admin/pendaftaran/ajax?show=diagnosa&s=' + encodeURIComponent(search) + '&t=1bdf2736be5c',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
  ...
  $('.propinsi').selectator({
    ...
    load: function (search, callback) {
      if (search.length < this.minSearchLength) return callback();
      $.ajax({
        url: 'http://localhost/lite3/admin/pendaftaran/ajax?show=propinsi&t=1bdf2736be5c',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
          callback(data['response']['list'].slice(0, 100));
  ...
  $('.kabupaten').selectator({
    ...
    load: function (search, callback) {
      if (search.length < this.minSearchLength) return callback();
      $.ajax({
        url: 'http://localhost/lite3/admin/pendaftaran/ajax?show=kabupaten&t=1bdf2736be5c',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
          callback(data['response']['list'].slice(0, 100));
